...
export const MessageCard = ({ message, username }) => {
  ...
  return (
    <div key={message._id} class={`flex items-start gap-2.5 ${message.sender.username == username ? "flex-row-reverse":""}`}>
      ...
      />
      <div
        class={`flex flex-col w-full max-w-[240px] leading-1.5 p-4 border-gray-200  ${
          message.sender.username === username
            ? "bg-gray-100 border-gray-200 rounded-s-xl rounded-b-xl"
            : " bg-indigo-500 border-indigo-600 text-white rounded-e-xl rounded-es-xl"
        }  dark:bg-gray-700`}
      >
        <div class="flex items-center space-x-2 rtl:space-x-reverse">
          ...
          <span class={`text-sm font-normal  dark:text-gray-400 ${ message.sender.username === username
            ? "text-gray-500 dark:text-white":" text-gray-300"}`}>
