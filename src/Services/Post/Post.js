...
class Post {
  createPost(body) {
    ...
    return axios.post(`${host_addr}/api/v1/posts/create-post`, body, {
      headers,
    });
  }

  uploadPostImage(body, setUploadProgress) {
    const accessTokenObject = JSON.parse(localStorage.getItem("access_token"));
    const headers = { Authorization: `Bearer ${accessTokenObject}` };
    return axios.post(`${host_addr}/api/v1/posts/upload-post-image`, body, {
      headers,
